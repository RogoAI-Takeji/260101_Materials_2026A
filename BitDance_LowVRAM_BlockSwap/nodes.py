...
class BitDanceFP8ScaledLinear(torch.nn.Module):
    ...
    def _dequant_weight(self, device: torch.device, dtype: torch.dtype) -> torch.Tensor:
        ...
        # Dequantize directly to target dtype (BF16) to avoid FP32 intermediate.
        # Peak VRAM: FP8(1x) + BF16_q(2x) + BF16_result(2x) = ~5x FP8 size.
        # Original FP32 path: FP8(1x) + FP32_q(4x) + FP32_q*scale(4x) = ~9x FP8 size.
        q = w.to(device=device, dtype=dtype)
        scale = s.to(device=device, dtype=dtype)
...
def _move_dynamic_cache_to(past_key_values, target_device):
    """Move a DynamicCache (or tuple KV) to target_device in-place. Returns it."""
    if past_key_values is None:
        return None
    if hasattr(past_key_values, "key_cache"):
        # HuggingFace DynamicCache format
        for j in range(len(past_key_values.key_cache)):
            kc = past_key_values.key_cache[j]
            vc = past_key_values.value_cache[j]
            if isinstance(kc, torch.Tensor) and kc.device != target_device:
                past_key_values.key_cache[j] = kc.to(target_device)
            if isinstance(vc, torch.Tensor) and vc.device != target_device:
                past_key_values.value_cache[j] = vc.to(target_device)
    elif isinstance(past_key_values, tuple):
        # Older tuple-of-(k,v) format — rebuild as list then freeze
        return tuple(
            (kv[0].to(target_device), kv[1].to(target_device)) if isinstance(kv, tuple) else kv
            for kv in past_key_values
        )
    return past_key_values


def _install_llm_streaming_hooks(base_llm, num_permanent_gpu_layers: int, device, llm_device):
    """
    Install PyTorch forward hooks to stream LLM transformer layers through GPU.

    Strategy
    --------
    * Layers 0 .. num_permanent_gpu_layers-1  → moved to GPU permanently.
    * Layers num_permanent_gpu_layers .. N-1  → kept on CPU; each layer is
      temporarily moved to GPU in its pre-hook and moved back in its post-hook.
    * KV cache (DynamicCache or tuple) lives on GPU throughout — it is small
      (< 1 GB for 768x1280) and keeps GPU compute latency low.
    * base_llm.norm is moved to GPU permanently (tiny parameters).

    Returns a list of hook handles for later removal.
    """
    layers = base_llm.layers
    hook_handles = []

    # Permanently place the first N layers on GPU
    for i in range(min(num_permanent_gpu_layers, len(layers))):
        layers[i].to(device)

    # Move the final layer-norm to GPU (tiny; needed at end of Qwen2Model.forward)
    if hasattr(base_llm, "norm") and base_llm.norm is not None:
        base_llm.norm.to(device)

    # Install streaming hooks for the remaining (CPU-resident) layers
    for i in range(num_permanent_gpu_layers, len(layers)):
        layer = layers[i]

        def _make_pre_hook(l):
            def _pre(module, args, kwargs):
                # Move layer weights to GPU for this forward pass
                l.to(device)
                # Move positional tensor args to GPU
                new_args = tuple(
                    a.to(device) if isinstance(a, torch.Tensor) else a
                    for a in args
                )
                # Move keyword args to GPU (handles tensors and DynamicCache)
                new_kwargs = {}
                for k, v in kwargs.items():
                    if isinstance(v, torch.Tensor):
                        new_kwargs[k] = v.to(device)
                    elif hasattr(v, "key_cache"):
                        # DynamicCache — move all cached tensors to GPU
                        _move_dynamic_cache_to(v, device)
                        new_kwargs[k] = v
                    elif (
                        ...
                        # Legacy (key, value) tuple
                        new_kwargs[k] = tuple(t.to(device) for t in v)
                    else:
                        new_kwargs[k] = v
        ...
        def _make_post_hook(l):
            def _post(module, args, output):
                # Return layer weights to CPU immediately after forward
                l.to(llm_device)
                return output
            return _post

        h1 = layer.register_forward_pre_hook(_make_pre_hook(layer), with_kwargs=True)
        h2 = layer.register_forward_hook(_make_post_hook(layer))
        hook_handles.extend([h1, h2])

    return hook_handles


def _remove_llm_streaming_hooks(base_llm, hook_handles, num_permanent_gpu_layers: int, llm_device):
    """Remove streaming hooks and move permanently-GPU layers back to CPU."""
    for h in hook_handles:
        h.remove()
    layers = base_llm.layers
    for i in range(min(num_permanent_gpu_layers, len(layers))):
        layers[i].to(llm_device)
    if hasattr(base_llm, "norm") and base_llm.norm is not None:
        base_llm.norm.to(llm_device)
...
class BitDanceSampler:
            ...
            "required": {
                ...
                "num_gpu_blocks": (
                    "INT",
                    {
                        ...
                        "step": 1,
                        "tooltip": (
                            "Block-swap: number of LLM transformer blocks to keep permanently on GPU. "
                            "0 = all blocks stream through GPU one-at-a-time (recommended for 16 GB). "
                            "Higher values keep more blocks on GPU permanently (faster, uses more VRAM; "
                            "each block ~280 MB for 14B FP8). KV-cache always lives on GPU."
                        ),
                    },
                ),
            },
    ...
    def sample(
        ...
        offload_device = comfy.model_management.unet_offload_device()
        # LLM weights stay on CPU; individual layers stream through GPU during inference
        llm_device = offload_device
        ...
        # Move LLM weights to CPU (block-swap: each layer streams to GPU during forward)
        LOGGER.info(
            "BitDanceSampler: LLM weights on CPU, streaming layers through GPU "
            "(num_gpu_blocks=%d permanently on GPU). KV-cache offloaded to CPU between blocks.", num_gpu_blocks
        )
        text_encoder.llm_model = text_encoder.llm_model.to(llm_device).eval()
        ...
        # Install block-swap streaming hooks: layers stream CPU→GPU→CPU during each forward
        _streaming_hooks = _install_llm_streaming_hooks(
            base_llm, num_gpu_blocks, device, llm_device
        )
        ...
        with torch.inference_mode(), torch.autocast(
            ...
            # With block-swap streaming, all LLM inputs live on GPU (layers stream to GPU)
            ...
            # embed_tokens is on CPU; move result to GPU for block-swap LLM input
            img_start_emb = embed_tokens(
                torch.tensor([img_start_id, res_h_id, res_w_id], device=llm_device, dtype=torch.long)
            ).to(device=device, dtype=autocast_dtype)

            for i in range(1, model.parallel_num):
                query_id = _token_id(tokenizer, f"<|query_{i}|>")
                query_embed = embed_tokens(
                    torch.tensor([query_id], device=llm_device, dtype=torch.long)
                ).to(device=device, dtype=autocast_dtype)
            ...
            _move_dynamic_cache_to(pkv_c, llm_device)  # offload KV to CPU to free VRAM

            # All LLM I/O on GPU (layers stream via hooks); bi_attn_mask on GPU
            ...
            pkv_c = outputs_c.past_key_values
            _move_dynamic_cache_to(pkv_c, llm_device)  # offload KV to CPU to free VRAM
            # hidden_c already on GPU (layers ran on GPU via streaming hooks)
            ...
            if guidance_scale > 1.0:
                ...
                _move_dynamic_cache_to(pkv_u, llm_device)  # offload KV to CPU to free VRAM
                ...
                pkv_u = outputs_u.past_key_values
                _move_dynamic_cache_to(pkv_u, llm_device)  # offload KV to CPU to free VRAM
                # hidden_u already on GPU (layers ran on GPU via streaming hooks)
            ...
            for step in range(num_steps):
                ...
                # model_input already on GPU; layers stream via hooks, KV-cache on GPU
                ...
                pkv_c = outputs_c.past_key_values
                _move_dynamic_cache_to(pkv_c, llm_device)  # offload KV to CPU to free VRAM
                # hidden_c on GPU (layers ran on GPU via streaming hooks)
                hidden_c = outputs_c.last_hidden_state[:, -step_width:]
                ...
                if guidance_scale > 1.0:
                    ...
                    pkv_u = outputs_u.past_key_values
                    _move_dynamic_cache_to(pkv_u, llm_device)  # offload KV to CPU to free VRAM
                    # hidden_u on GPU (layers ran on GPU via streaming hooks)
                    hidden_u = outputs_u.last_hidden_state[:, -step_width:]
        ...
        # Remove streaming hooks and move any permanently-GPU layers back to CPU
        _remove_llm_streaming_hooks(base_llm, _streaming_hooks, num_gpu_blocks, llm_device)
