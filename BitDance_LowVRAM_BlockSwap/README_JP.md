...
1. `nodes.py`を`custom_nodes/Comfyui-bitdance/`フォルダに上書きコピー
2. ComfyUIを再起動
3. `BitDance_14B_64x_portrait.json`をComfyUIに読み込む
4. BitDanceSamplerノードの`num_gpu_blocks`を15に設定（VRAMに合わせて調整）
5. 実行
...
- ブロックスワップ実装：[RogoAI-Takeji](https://github.com/RogoAI-Takeji) / YouTube：[老後AI (RogoAI)](https://www.youtube.com/@RogoAI)
